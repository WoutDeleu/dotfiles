...
CUSTOM_CSS_FILE = os.path.expanduser('~/.config/waybar-ycal/style.css')
...
class CalendarPopup(Gtk.ApplicationWindow):
    ...
    def _build_ui(self):
        ...
        self._refresh_da.set_size_request(26, 26)
        ...
        self.refresh_btn.set_size_request(34, 34)
        ...
        self.right_box.set_size_request(230, -1)
    ...
    def _build_grid(self):
        ...
        for i in range(total_days):
            ...
            overlay = Gtk.Overlay()
            overlay.set_size_request(34, 34)
    ...
    def _draw_refresh_icon(self, da, ctx, width, height):
        ...
        if not hasattr(self, '_refresh_layout'):
            ...
            self._refresh_layout.set_font_description(
                Pango.FontDescription.from_string(f"{font_name} 16"))
    ...
    def _apply_css(self):
      ...
      """.encode()
        ...
        if os.path.exists(CUSTOM_CSS_FILE):
            if not hasattr(self, '_custom_css_provider'):
                self._custom_css_provider = Gtk.CssProvider()
                Gtk.StyleContext.add_provider_for_display(
                    self.get_display(), self._custom_css_provider, Gtk.STYLE_PROVIDER_PRIORITY_USER
                )
            try:
                self._custom_css_provider.load_from_path(CUSTOM_CSS_FILE)
            except Exception:
                pass
