...
| 6 | **waybar-gcal OAuth client secret** | `~/.config/waybar/google-calendar/client_secret.json` (`600`) | Google Cloud OAuth desktop client JSON; tracked in `waybar/` stow package but gitignored. `WAYBAR_GCAL_CLIENT_SECRET` env var points here (set in `hypr/decoration.conf`). After deploy, run `waybar-gcal auth` to generate token. | ⬜ vault TODO |
